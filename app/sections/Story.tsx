...
function Chapter({ chapter, index }: { chapter: ChapterType; index: number }) {
      ...
      <div style={{ textAlign: 'center', padding: '100px 6vw 60px' }}>
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <h2 style={{ fontSize: 'clamp(2rem,4vw,3.2rem)', fontWeight: 900, letterSpacing: '-1px', marginBottom: 20 }}>
            {t('story_headline')}<br />
            <span style={{ background: 'linear-gradient(135deg,#4F6EF7,#a855f7)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{t('story_headline2')}</span>
          </h2>
